...
function makeFuncParamInsertTask(connection, funcs, key, value) {
	return function(callback) {
		var sql = 'INSERT INTO config.SIGNAL_TRANS_FUNC_PROP (OBJECT_ID,SIGNAL_ID,PROP,VALUE) values(?,?,?,?)';
		connection.query(sql, [ funcs.OBJECT_ID, funcs.SIGNAL_ID, key, value ], function(err, result) {
			if (err) {
				callback(err);
			} else {
				callback();
...
function makeFuncsCreateTasks(connection, tasks, objectId, signalId, funcs) {
	...
	for (key in params) {
		tasks.push(makeFuncParamInsertTask(connection, funcs, key, params[key]));
	}
}
