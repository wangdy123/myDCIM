...
function doTransaction(taskCreator, callback) {
...
module.exports.getSequence = function(seqName, callback) {
	var seq = 0;
	doTransaction(function(connection) {
		return [ function(cbk) {
			var sql = 'select VALUE from config.SEQUENCES where SEQ_NAME=?';
			connection.query(sql, [ seqName ], function(err, result) {
				if (err) {
					cbk(err);
				} else {
					seq = 0;
			...
			var sql = 'replace INTO config.SEQUENCES(SEQ_NAME,VALUE)values(?,?)';
			connection.query(sql, [ seqName, seq ], function(err, result) {
				if (err) {
					cbk(err);
				} else {
					cbk(null);
