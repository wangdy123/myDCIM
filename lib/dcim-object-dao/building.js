...
module.exports.createInsertTasks = function(connection, tasks, obj) {
	tasks.push(function(objectId, callback) {
		var sql = 'INSERT INTO config.BUILDING(ID,FLOOR_UNDERGROUND,FLOOR_GROUND,DESCRIPTION)'
				+ 'values(?,?,?,?)';
		connection.query(sql, [ objectId, obj.FLOOR_UNDERGROUND, obj.FLOOR_GROUND, obj.DESCRIPTION ],
				function(err, result) {
...
module.exports.createUpdateTasks = function(connection, tasks, obj) {
	tasks.push(function(objectId, callback) {
		var sql = 'update config.BUILDING set FLOOR_UNDERGROUND=?,FLOOR_GROUND=?,DESCRIPTION=? where ID=?';
		connection.query(sql, [ obj.FLOOR_UNDERGROUND, obj.FLOOR_GROUND, obj.DESCRIPTION, objectId ],
				function(err, result) {
					callback(err, objectId);
				});
	});
