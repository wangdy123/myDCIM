$(function() {
    const publisherName = 'apps';
    const selfDiagnosisUrl = 'selfDiagnosis';
    const objectNodeUrl = 'logicobject/objectNodes/';
    const currentAccountUrl = 'currentAccount';
    const themesUrl = 'account/themes';
    const passwordUrl = 'setPassword';
    let currentLogicObject = null;
    ...
    WUI.initI18n(WUI.initValidator);
    ...
    $('#change-password-btn').click(function() {
        WUI.showDialog($('#change-password-dialog'),{
            iconCls : 'icon-key',
            title : WUI.getI18n('string_change_password'),
            defaultHeight : 400,
            width : 350,
            ...
            onLoad : function() {
                ...
                $('#account-password-confirm').textbox({
                    ...
                    invalidMessage : WUI.getI18n('validate_confirm_password')
                });
            },
            buttons : [ {
                text : WUI.getI18n('string_save'),
                handler : function() {
                    let isValid = $('#account-password').textbox('isValid');
                    ...
                    const password = {
                        OLD_PASSWORD : $('#old-password').textbox('getValue'),
                        NEW_PASSWORD : $('#account-password').textbox('getValue')
                    };
                    WUI.ajax.put(passwordUrl, password, function() {
                        $.messager.alert(WUI.getI18n('string_success'), WUI.getI18n('string_change_success'));
                        $('#change-password-dialog').dialog('close');
                    }, function() {
                        $.messager.alert(WUI.getI18n('string_fail'), WUI.getI18n('string_change_fail'));
                    });
                }
            }, {
                text : WUI.getI18n('string_cancel'),
                handler : function() {
                    $('#change-password-dialog').dialog('close');
                }
            } ]
    ...
    $('#current-user-panel').click(function() {
        WUI.ajax.get(currentAccountUrl, {}, function(account) {
            WUI.showDialog($('#change-password-dialog'),{
                iconCls : 'icon-man',
                title : WUI.getI18n('string_change_currentuser'),
                defaultHeight : 400,
                width : 350,
                href : 'account-detal-dialog.html',
                onLoad : function() {
                    const menus = [];
                    WUI.menus.forEach(function(childMenus){
                        for (var j = 0; j < childMenus.length; j++) {
                            childMenus[j].group = childMenus.name;
                    ...
                    if (account.IS_GOD) {
                        $('#account-role').text(WUI.getI18n('string_master'));
                    } else {
                        const roles = [];
    ...
    const $selfDiagnosisSoundBox = $(document.createElement('div'));
    ...
    function selfDiagnosis() {
        ...
        WUI.ajax.get(selfDiagnosisUrl, {}, function(status) {
            ...
            if (status.length > 0) {
                $('#self-diagnosis-content').html(status.join(',') +  WUI.getI18n('string_self_diagnosis'));
            ...
            WUI.selfDiagnosisTimer = setTimeout(selfDiagnosis, WUI.requestInteval.selfDiagnosis);
            $('#self-diagnosis-content').html(WUI.getI18n('string_web_service_fail'));
            $('#self-diagnosis-tooltip').dialog('open');
            beginSelfDiagnosisSound();
        });
    }
    ...
    function checkDialogActive(maxSequece) {
        const lastSeq = $.cookie('maxSequece');
    ...
    function setAlarmCount(activeAlarmCount) {
        ...
        $('#alarm-count-status').text(WUI.getI18n('string_alarm_count' , totalCount));
    }

    const $alarmSoundBox = $(document.createElement('div'));
    $('body').append($alarmSoundBox);
    let g_alarmSoundFile = null;
    function checkAlarmSound(unAckActiveAlarmCount) {
        ...
        const shoundFile = `level${maxLevel}.wav`;
    ...
    $('#self-diagnosis-sound').click(function() {
        const muted = !$.cookie('self-diagnosis-muted');
        if (muted) {
