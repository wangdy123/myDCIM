									...
									columns : [ [
											{
												...
												width : 100,
												formatter : function(value, row, index) {
													var model = findModel(row.CABINET_MODEL);
								...
								onLoad : function() {
									for (var index = 0; index < models.length; index++) {
										$('#cabinet-model-txt').append(
												'<option value="' + models[index].ID + '">' + models[index].NAME
														+ '</option>');
									}
									function setEndDate() {
										var startDate = $('#start-use-date').datebox('getValue');
										var model = findModel($('#cabinet-model-txt').val());
										console.log("startDate" + $('#cabinet-model-txt').val() + ",model" + model);
										if (startDate && model) {
											var endDate = WUI.timeAddSecond(startDate,
													parseInt(model.MAX_USE_YEAR, 10) * 365);
