$(function() {
    const publisherName = 'map';
    const objectLocationUrl = 'monitor/objectLocations/';
    const objectNodeUrl = 'logicobject/objectNodes/';
    const iconUrl = 'monitor/map-icon/';

    const mapPicDir = 'roadmap/';
    ...
    map.mapTypes.set('localMap', {
        ...
        getTile : function(coord, zoom, ownerDocument) {
            ...
            const curSize = Math.pow(2, zoom);
            const strURL = mapPicDir + zoom + '/' + (coord.x % curSize) + '/' + (coord.y % curSize) + '.png';
            img.src = strURL;
            return img;
        }
    }); // 绑定本地地图类型
    ...
    function markObject(param) {
        WUI.openNodeSelectDialog($('#map-object-select-dialog'), {
            onSelect : function(data) {
                data.LONGITUDE = param.latLng.e;
                data.LATITUDE = param.latLng.d;
                WUI.ajax.put(objectLocationUrl + data.ID, data, function() {
                    addMarker(data);
                }, function() {
    ...
    function getObjectContent(objectLocation) {
        return `<table class="map-tooltip">
                        <tr><td colspan="8"><b style="font-size:18px;lineheight:24px;">${objectLocation.NAME}</b></td></tr>
                        <tr><td align="right" colspan="6"><label>节点类型：</label></td><td>${getMakerPro(objectLocation).name}</td></tr>
                        <tr><td align="right" colspan="6"><label>编码：</label></td><td>${objectLocation.CODE}</td></tr>
                        <tr><td align="right" colspan="6"><label>数据中心机楼：</label></td><td>${objectLocation.buildingCount}</td></tr>
                        <tr><td align="right" colspan="6"><label>数据中心机房：</label></td><td>${objectLocation.roomCount}</td></tr>
                        <tr><td align="right" colspan="6"><label>总机架数：</label></td><td>${objectLocation.cabinetCount}</td></tr>
                        <tr><td colspan="8"><label>该区域告警统计</label></td></tr>
                        <tr>
                            <td><div class="alarmLevel1-icon" title="一级告警"></div></td><td>${objectLocation.alarmLevel1Count}</td>
                            <td><div class="alarmLevel2-icon" title="二级告警"></div></td><td>${objectLocation.alarmLevel2Count}</td>
                            <td><div class="alarmLevel3-icon" title="三级告警"></div></td><td>${objectLocation.alarmLevel3Count}</td>
                            <td><div class="alarmLevel4-icon" title="四级告警"></div></td><td>${objectLocation.alarmLevel4Count}</td>
                        </tr>
                      </table>`;
    }
    ...
    function addMarker(objectLocation) {
        ...
        const infowindow = setInfoWindow(marker, objectLocation);
    ...
    function setInfoWindow(marker, objectLocation) {
        ...
        const infowindow = new google.maps.InfoWindow({// 定义一个窗口，当点击时弹出该窗口
    ...
    function openObject(object, needPublish, callback) {
        if (needPublish) {
            WUI.ajax.get(objectNodeUrl + object.ID, function(fullObject) {
        ...
        WUI.ajax.get(objectLocationUrl + objectId, {}, function(objectLocation) {
            if (!objectLocation.LONGITUDE || !objectLocation.LATITUDE) {
                return;
            }
