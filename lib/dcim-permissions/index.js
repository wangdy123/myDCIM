		...
		module.exports.getCurrentUser(req, res, function(err, result) {
			if (err) {
				...
				if (checkAuth(result, req.url, req.method)) {
					...
					logger.authLog("user:" + result.NAME + ",url:" + req.url + ",method:" + req.method + " not allow");
...
module.exports.initLogin = function(app) {
	app.post('/login', function(req, res) {
		getaccountByNamePassword(db.pool, req.body.username, req.body.password, function(error, account) {
			...
			getAccountRight(db.pool, account.ID, function(error, right) {
				...
				cache.set(ssid, account, function(error, result) {
					if (error) {
						logger.authLog(error);
						res.status(400).send("登录出错，请重试");
						return;
	...
	app.put('/currentAccount', function(req, res) {
		module.exports.getCurrentUser(req, res, function(err, account) {
			if (err) {
				res.status(400).send(err);
			} else {
				account.DEFAULT_THEME = req.body.DEFAULT_THEME;
				account.HOME_PAGE = req.body.HOME_PAGE;
				account.JOB_NUMBER = req.body.JOB_NUMBER;
				account.E_MAIL = req.body.E_MAIL;
				...
				db.doTransaction(function(connection) {
					var tasks = [
							function(callback) {
								...
								connection.query(sql, [ req.body.DEFAULT_THEME, req.body.HOME_PAGE, account.ID ],
										function(err, result) {
											if (err) {
												callback(err);
											} else {
							...
							function(callback) {
								var sql = 'update portal.PERSONNEL_CFG set JOB_NUMBER=?,E_MAIL=?,TEL=? where ID=?';
								connection.query(sql,
										[ req.body.JOB_NUMBER, req.body.E_MAIL, req.body.TEL, account.ID ], function(
												err, result) {
											if (err) {
												callback(err);
											} else {
								...
								cache.set(req.cookies.ssid, account, function(err, result) {
									if (err) {
										callback(err);
									} else {
										callback(null);
