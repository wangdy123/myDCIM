...
module.exports.getByPositionParent = function(pool, parentId, callback) {
	var sql = 'select o.ID,o.NAME,o.OBJECT_TYPE,p.PARENT_ID,c.CODE,c.CABINET_MODEL,c.SEQUENCE,c.CABINET_DEPTH,'
			+ 'c.START_USE_DATE,c.EXPECT_END_DATE from config.OBJECT o join config.POSITION_RELATION p on o.ID=p.ID '
			+ 'join config.CABINET c on o.ID=c.ID where p.PARENT_ID=?';
...
module.exports.getById = function(pool, id, callback) {
	var sql = 'select o.ID,o.NAME,o.OBJECT_TYPE,p.PARENT_ID,c.CODE,c.CABINET_MODEL,c.SEQUENCE,'
			+ 'c.CABINET_DEPTH,c.START_USE_DATE,c.EXPECT_END_DATE from config.OBJECT o '
			+ ' join config.POSITION_RELATION p on o.ID=p.ID join config.CABINET c on o.ID=c.ID where o.ID=?';
...
module.exports.createInsertTasks = function(connection, tasks, obj) {
	tasks.push(function(objectId, callback) {
		var sql = 'INSERT INTO config.CABINET(ID,CODE,CABINET_MODEL,SEQUENCE,CABINET_DEPTH,'
				+ 'START_USE_DATE,EXPECT_END_DATE) values(?,?,?,?,?,?,?)';
		connection.query(sql, [ objectId, obj.CODE, obj.CABINET_MODEL, obj.SEQUENCE, obj.CABINET_DEPTH,
				obj.START_USE_DATE, obj.EXPECT_END_DATE ], function(err, result) {
...
module.exports.createUpdateTasks = function(connection, tasks, obj) {
	tasks.push(function(objectId, callback) {
		var sql = 'update config.CABINET set CODE=?,CABINET_MODEL=?,SEQUENCE=?,CABINET_DEPTH=?,'
				+ 'START_USE_DATE=?,EXPECT_END_DATE=? where ID=?';
		connection.query(sql, [ obj.CODE, obj.CABINET_MODEL, obj.SEQUENCE, obj.CABINET_DEPTH, obj.START_USE_DATE,
