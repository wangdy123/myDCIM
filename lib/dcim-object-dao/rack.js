...
module.exports.getByPositionParent = function(pool, parentId, callback) {
	var sql = 'select o.ID,o.NAME,o.CODE,o.OBJECT_TYPE,p.PARENT_ID,c.RACK_MODEL,c.SEQUENCE,c.RACK_DEPTH,'
			+ 'c.START_USE_DATE,c.EXPECT_END_DATE from config.OBJECT o join config.POSITION_RELATION p on o.ID=p.ID '
			+ 'left join config.RACK c on o.ID=c.ID where p.PARENT_ID=? and o.OBJECT_TYPE=?';
	pool.query(sql, [ parentId, objectTypeDef.RACK ], function(error, objects, fields) {
		if (error) {
			callback(error);
			return;
		}
...
module.exports.getById = function(pool, id, callback) {
	var sql = 'select o.ID,o.NAME,o.CODE,o.OBJECT_TYPE,p.PARENT_ID,c.RACK_MODEL,c.SEQUENCE,'
			+ 'c.RACK_DEPTH,c.START_USE_DATE,c.EXPECT_END_DATE from config.OBJECT o '
			+ 'left join config.POSITION_RELATION p on o.ID=p.ID join config.RACK c on o.ID=c.ID '
			+ 'where o.ID=? and o.OBJECT_TYPE=?';
	pool.query(sql, [ id, objectTypeDef.RACK ], function(error, objects, fields) {
		if (error) {
			callback(error);
			return;
		}
		if (objects.length === 0) {
			callback("not found RACK:" + id);
			return;
		}
		transform(objects);
...
module.exports.createInsertTasks = function(connection, tasks, obj) {
	tasks.push(function(objectId, callback) {
		var sql = 'INSERT INTO config.RACK(ID,RACK_MODEL,SEQUENCE,RACK_DEPTH,'
				+ 'START_USE_DATE,EXPECT_END_DATE) values(?,?,?,?,?,?)';
		connection.query(sql, [ objectId, obj.RACK_MODEL, obj.SEQUENCE, obj.RACK_DEPTH,
...
module.exports.createUpdateTasks = function(connection, tasks, obj) {
	tasks.push(function(objectId, callback) {
		var sql = 'REPLACE INTO config.RACK(ID,RACK_MODEL,SEQUENCE,RACK_DEPTH,'
				+ 'START_USE_DATE,EXPECT_END_DATE) values(?,?,?,?,?,?)';
		connection.query(sql, [ objectId, obj.RACK_MODEL, obj.SEQUENCE, obj.RACK_DEPTH,
				util.date_parse(obj.START_USE_DATE), util.date_parse(obj.EXPECT_END_DATE) ], function(err, result) {
			callback(err, objectId);
...
module.exports.createDeleteTasks = function(connection, tasks, objectId) {
	tasks.push(function(callback) {
		var sql = 'delete from config.RACK where ID=?';
